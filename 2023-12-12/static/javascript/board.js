// board.js
// https://fontawesome.com/account<< 폰트사이트
//플레이어 생성자 함수
...
function game_init(){
    ...
    // 게임 플레이어수에 맞는 말 만들기 
    // svg - scalable vector graphiscs ( 수학공식을 통해 이미지를 저장 ,표시 )
    // <i class="fa-duotone fa-wreath-laurel"></i>
}
function change_pcl(){ // 플레이어가 자신의 말 색상을 변경할 경우 실행
 var new_color = $(this).val();
 var num = Number($(this).attr("id").substring(3)); //attr 은 태그의 속성 - id
 var gamer = player_list[num-1]; // 색상 변경한 플레이어 객체
}
gamer.color=new_color;
// 자신의 말 찾기, num변수에는 플레이어의 번호가 저장되있다.
...
$(function(){
    ...
    // getJSON으로 json 파일을 읽어와서 zone배열에 저장
    // zone 배열의 내용은 어제와 동일하다.
    // zone_draw() 함수는 zone 배열의 값을 가지고와서 화면에 표시한다.
    // 하지만 zone_draw() 함수가 정상적으로 동작하ㅣ 못하고 있다.
    // getJSON function(data) { } 안으로 zone_draw() 함수를 넣어주면
    // 정상적으로 실행된다.
    // 비동기문제 발생!!!
