...
function result(){
    ...
    // .prop("disabled") -> 비활성화면 true, 활성화면 flase
...
function start(){ // 컴퓨터 가위바위보 이미지 보이기, }
    com = setInterval( imggoate , 100)  // 1000이 1초이다 컴퓨터 이미지 돌리기
    $("#start").text("게임중");
...
function imggoate(){
    ...
    if(comIdx==3) comIdx=0;
    // comIdx는 컴퓨터 이미지 표현하기 위한 인덱스 변수다
    // comIdx는 값이 2가 될때는 화면에 보자기가 출력 된 후이기 때문에 
    // comIdx를 -1로 변경한다.
    // ++comIdx에 의해 0으로 증가하여 0번인 덱스인 가위 이미지가 출력
    $( "#comImg").attr("src", path+ gameImg[++comIdx]);
}